...
void editorUpdateSyntax(erow *row) {
    ...
    memset(row->hl, HL_NORM, row->size);
    ...
    while (i < row->rsize) {
        char c = row->render[i];
        unsigned char prev_hl = (i > 0) ? row->hl[i - 1] : HL_NORM;
        ...
        if (E.syntax->flags & HL_HIGHLIGHT_NUMBERS) {
            if ((isdigit(c) && (prev_sep || prev_hl == HL_NUM)) || 
                (c == '.' && prev_hl == HL_NUM)) {
                row->hl[i] = HL_NUM;
                i++;
                prev_sep = 0;
            ...
            for (j = 0; keywords[j]; j++) {
                ...
                if (!strncmp(&row->render[i], keywords[j], klen) && 
                    is_separator(row->render[i + klen])) {
                        memset(&row->hl[i], kw2 ? HL_KEY2 : HL_KEY1, klen);
                        i += klen;
                        break;
                }
            }
...
int editorSyntaxToColor(int hl) {
    switch (hl) {
        ...
        case HL_KEY1: return 33;
        case HL_KEY2: return 32;
        case HL_STR: return 35;
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t') {
            curr_rx += (EDITOR_TAB_STOP - 1) - (curr_rx % EDITOR_TAB_STOP);
        }
        curr_rx++;
...
void editorUpdateRow(erow *row) {
    /* Function that renders tab characters*/
    int tabs = 0;
    int j;
...
int main(int argc, char *argv[]) {
    ...
    editorSetStatusMessage("HELP: Ctrl-S = save | Ctrl-Q to quit | Ctrl-F = search");
